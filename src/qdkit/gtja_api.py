...
_api_url = 'https://vip.gtjaqh.com:443/api/'

_api_futures_contract = _api_url + 'unicorn.cloudApi.contractTradeParams.query.do'  # 合约参数查询
_api_futures_prices = _api_url + 'unicorn.cloudApi.futuresContractPrice.queryByCode.do'  # 期货合约价格查询
...
def _api_query(api_url, params):
    ...
    # return df.rename(columns={col: _camel_to_snake(col) for col in df.columns})
    return df
...
def get_futures_contracts(date=None):
    ...
    for e in gj_exchanges:
        result = _api_query(_api_futures_contract, {'tradingDay': _get_date_str(date), 'exchangeCode': e})
...
def get_futures_prices(date=None):
    """查询指定日期期货合约价格"""
    return _api_query(_api_futures_prices, {'tradingDay': _get_date_str(date), 'exchangeCode': None})


def get_futures_basis(code=None, start_date=None, end_date=None):
    """查询期货基差数据，code 为合约代码，不传则查全部"""
    return _api_query(_api_futures_basis, {
        'code': code,
